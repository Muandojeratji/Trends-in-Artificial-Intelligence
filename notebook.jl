...
# ╔═╡ 887336e0-1185-11f0-05ca-2f87d42f4a28
using GLM

# ╔═╡ d6f017ce-a18f-467c-9cd8-5cffbb57c536
using CSV

# ╔═╡ 524ea476-d5bb-4818-a500-0979055be24e
using Plots

# ╔═╡ 51b66210-d5ef-4b51-a2c2-ff8d0b0ec562
using DataFrames

# ╔═╡ 0ac1ddfc-ec34-4024-8a39-ea1655ae49dc
using MLJ

# ╔═╡ a4c208c6-ac68-4c90-b22d-b922c776e794
using Zygote

# ╔═╡ 06cfcb37-28f1-4c8a-96e6-ddafab089c1d
using MLDatasets

# ╔═╡ 0c806299-bde4-4ae7-b8c7-be20aa4cde26
using Flux : onehotbatch

# ╔═╡ d7dd65e3-f3de-49c5-951c-5c31a19a51f9
# ╠═╡ disabled = true
#=╠═╡
using Flux
  ╠═╡ =#

# ╔═╡ 901c85a1-1491-4734-9df3-91ba9984e07c
import MLJLinearModels

# ╔═╡ 196781e8-87cd-47b7-ac43-18f3ecd67590
import Optimisers

# ╔═╡ 1b1df00c-73fb-44bb-b6db-ed46aba17002
begin
    # Define target variable column symbols
    target_fruit_loss = :economic_loss_fruit
    target_dairy_waste = :household_waste_dairy
end


# ╔═╡ 127d31c7-29f5-4ca0-9718-2c0ceb908563
### Cell 2: Define DataFrame (df) and split
train_fraction = 0.8

# ╔═╡ 93f4c91f-c8ea-4020-a064-b2107edbb866
first(df, 5)  # Preview first 5 rows

# ╔═╡ 7604beda-c1c0-40d3-b59b-63d0bb7209e7
count(col -> any(ismissing, df[!, col]), names(df))

# ╔═╡ 83a6117e-b8c7-4362-8e5e-f4568aff11f0
describe(df)  # Summary statistics

# ╔═╡ f1bdd558-ab22-47ba-b905-ae5a06dd02cf
# Histograms for numeric features
for col in names(df)
    if eltype(df[!, col]) <: Number
        histogram(df[!, col], title=col, bins=30)
    end
end


# ╔═╡ 25310aee-1e88-4d4c-9e08-7c6bb4179838
begin
    plots = []

    for col in names(df)
        if eltype(df[!, col]) <: Number
            push!(plots, histogram(df[!, col], title=col, bins=30))
        end
    end

    plot(plots..., layout = (length(plots), 1), size=(600, 300 * length(plots)))
end


# ╔═╡ 2d95c91f-9dd0-401d-b988-8a72b2b69b35
begin
    # Pearson correlation function
    function pearson_correlation(x, y)
        x̄, ȳ = mean(x), mean(y)
        num = sum((x .- x̄) .* (y .- ȳ))
        den = sqrt(sum((x .- x̄).^2) * sum((y .- ȳ).^2))
        return num / den
    end
    ...
    for i in 1:n
        for j in 1:n
            corr_matrix[i, j] = pearson_correlation(numeric_df[!, cols[i]], numeric_df[!, cols[j]])
        end
    end
    ...
    heatmap(
        corr_matrix,
        title = "Correlation Heatmap (Manual)",
        xlabel = "Variables",
        ylabel = "Variables",
        xticks = (1:n, cols),
        yticks = (1:n, cols),
...
# ╔═╡ 922056ab-e774-4bb2-bc4b-30419d6c5763
df = DataFrame(A = 1:100, B = rand(100))  # Example DataFrame

# ╔═╡ 3e7d2c61-3d69-483a-b5b9-a2fcb3e62f3f
# ╠═╡ disabled = true
#=╠═╡
df = CSV.read("C:/Users/BOBBY T KATJIWE/Downloads/global_food_wastage_dataset.csv", DataFrame)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "a5d1197821220354a6bf757a8d1e0973d4dfd577"
